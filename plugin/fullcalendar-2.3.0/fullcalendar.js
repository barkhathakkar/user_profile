...
var fc = $j.fullCalendar = { version: "2.3.0" };
var fcViews = fc.views = {};


$j.fn.fullCalendar = function(options) {
	var args = Array.prototype.slice.call(arguments, 1); // for a possible method call
	var res = this; // what this function will return (this jQuery object by default)

	this.each(function(i, _element) { // loop each DOM element involved
		var element = $j(_element);
		var calendar = element.data('fullCalendar'); // get the existing calendar object (if any)
		var singleRes; // the returned value of this single method call

		// a method call
		if (typeof options === 'string') {
			if (calendar && $j.isFunction(calendar[options])) {
...
function mergeOptions() {
	...
	for (i = 0; i < complexOptions.length; i++) {
		...
		for (j = 0; j < chain.length; j++) {
			...
			if ($j.isPlainObject(val)) {
				combinedVal = $j.extend(combinedVal || {}, val); // merge new properties
			}
	...
	chain.unshift({}); // $j.extend will mutate this with the result
	chain.push(complexVals); // computed complex values are applied last
	return $j.extend.apply($j, chain); // combine
}
...
function massageOverrides(input) {
	...
	$j.each(input, function(name, val) {
		if (name != 'views') {

			// could the value be a legacy View-Option-Hash?
			if (
				$j.isPlainObject(val) &&
				!/(time|duration|interval)$j/i.test(name) && // exclude duration options. might be given as objects
				$j.inArray(name, complexOptions) == -1 // complex options aren't allowed to be View-Option-Hashes
			) {
				subObj = null;

				// iterate through the properties of this possible View-Option-Hash value
				$j.each(val, function(subName, subVal) {

					// is the property targeting a view?
					if (/^(month|week|day|default|basic(Week|Day)?|agenda(Week|Day)?)$j/.test(subName)) {
...
function disableCursor() {
	$j('body').addClass('fc-not-allowed');
}


// Returns the mouse cursor to its original look
function enableCursor() {
	$j('body').removeClass('fc-not-allowed');
}
...
function distributeHeight(els, availableHeight, shouldRedistribute) {
	...
	els.each(function(i, el) {
		...
		var naturalOffset = $j(el).outerHeight(true);

		if (naturalOffset < minOffset) {
			flexEls.push(el);
			flexOffsets.push(naturalOffset);
			flexHeights.push($j(el).height());
		}
		else {
...
function matchCellWidths(els) {
	...
	els.find('> *').each(function(i, innerEl) {
		var innerWidth = $j(innerEl).outerWidth();
...
function isAtomic(val) {
	return /undefined|null|boolean|number|string/.test($j.type(val));
}


function applyAll(functions, thisObj, args) {
	if ($j.isFunction(functions)) {
		functions = [ functions ];
	}
	if (functions) {
		var i;
...
function cssToStr(cssProps) {
	...
	$j.each(cssProps, function(name, val) {
		if (val != null) {
...
var ambigDateOfMonthRegex = /^\s*\d{4}-\d\d$j/;
var ambigTimeOrZoneRegex =
	/^\s*\d{4}-(?:(\d\d-\d\d)|(W\d\d$j)|(W\d\d-\d)|(\d\d\d))((T| )(\d\d(:\d\d(:\d\d(\.\d+)?)?)?)?)?$j/;
var newMomentProto = moment.fn; // where we will attach our new methods
var oldMomentProto = $j.extend({}, newMomentProto); // copy of original moment methods
...
var Popover = Class.extend({
	...
	render: function() {
		...
		this.el = $j('<div class="fc-popover"/>')
		...
		if (options.autoHide) {
			$j(document).on('mousedown', this.documentMousedownProxy = proxy(this, 'documentMousedown'));
		}
	},
	...
	getCell: function(x, y) {
		...
		if (this.inBounds(x, y)) {
			...
			if (hitRow !== null && hitCol !== null) {
				...
				$j.extend(cell, rowCoords[hitRow], colCoords[hitCol]);
	...
	startListening: function(ev) {
		...
		if (!this.isListening) {
			...
			if (ev && this.options.scroll) {
				scrollParent = getScrollParent($j(ev.target));
...
var MouseFollower = Class.extend({
	...
	constructor: function(sourceEl, options) {
		...
		this.parentEl = options.parentEl ? $j(options.parentEl) : sourceEl.parent(); // default to sourceEl's parent
	},
	...
	start: function(ev) {
		if (!this.isFollowing) {
			...
			$j(document).on('mousemove', this.mousemoveProxy = proxy(this, 'mousemove'));
		}
	},
	...
	stop: function(shouldRevert, callback) {
		...
		if (this.isFollowing && !this.isAnimating) { // disallow more than one stop animation at a time
			...
			$j(document).off('mousemove', this.mousemoveProxy);
...
var Grid = fc.Grid = RowRenderer.extend({
	...
	getCell: function(row, col) {
		...
		$j.extend(cell, this.getRowData(row), this.getColData(col));
		$j.extend(cell, this.computeCellRange(cell));
	...
	setElement: function(el) {
		...
		el.on('mousedown', function(ev) {
			if (
				!$j(ev.target).is('.fc-event-container *, .fc-more') && // not an an event element, or "more.." link
				!$j(ev.target).closest('.fc-popover').length // not on a popover (like the "more.." events one)
			) {
	...
	bindGlobalHandlers: function() {
		$j(document).on('dragstart sortstart', this.externalDragStartProxy); // jqui
	},


	// Unbinds DOM handlers from elements that reside outside the grid
	unbindGlobalHandlers: function() {
		$j(document).off('dragstart sortstart', this.externalDragStartProxy); // jqui
	},
		...
		if (segs.length) {
			...
			$j(html).each(function(i, node) {
				...
				if (el) { // custom filters did not cancel the render
					el = $j(el); // allow custom filter to return raw DOM node
...
Grid.mixin({
	...
	renderFgSegEls: function(segs, disableResizing) {
		...
		if (segs.length) { // don't build an empty html string
			...
			$j(html).each(function(i, node) {
				var seg = segs[i];
				var el = view.resolveEventEl(seg.event, $j(node));
		...
		$j.each(
			{
				...
				mousedown: function(seg, ev) {
					if ($j(ev.target).is('.fc-resizer') && view.isEventResizable(seg.event)) {
						_this.segResizeMousedown(seg, ev, $j(ev.target).is('.fc-start-resizer'));
					}
			...
			function(name, func) {
				...
				_this.el.on(name, '.fc-event-container > *', function(ev) {
					var seg = $j(this).data('fc-seg'); // grab segment data. put there by View::renderEvents
	...
	externalDragStart: function(ev, ui) {
		...
		if (view.opt('droppable')) { // only listen if this setting is on
			el = $j((ui ? ui.item : null) || ev.target);

			// Test that the dragged element passes the dropAccept selector or filter function.
			// FYI, the default is "*" (matches all)
			accept = view.opt('dropAccept');
			if ($j.isFunction(accept) ? accept.call(el[0], el) : el.is(accept)) {
	...
	eventsToRanges: function(events) {
		...
		$j.each(eventsById, function(id, eventGroup) {
...
function getDraggedElMeta(el) {
	...
	if (eventProps) {
		if (typeof eventProps === 'object') {
			eventProps = $j.extend({}, eventProps); // make a copy
		}
...
var DayGrid = Grid.extend({
	...
	renderHelper: function(event, sourceSeg) {
		...
		this.rowEls.each(function(row, rowNode) {
			var rowEl = $j(rowNode); // the .fc-row
			var skeletonEl = $j('<div class="fc-helper-skeleton"><table/></div>'); // will be absolutely positioned
			var skeletonTop;
		...
		this.helperEls = $j(helperNodes); // array -> jQuery set
	},
...
DayGrid.mixin({
	...
	renderBgSegs: function(segs) {
		...
		var allDaySegs = $j.grep(segs, function(seg) {
	...
	renderFgSegs: function(segs) {
		...
		this.rowEls.each(function(i, rowNode) {
			$j(rowNode).find('.fc-content-skeleton > table').append(
		...
		for (i = 0; i < levelCnt; i++) { // iterate through all levels
			...
			if (levelSegs) {
				for (j = 0; j < levelSegs.length; j++) { // iterate through segments in level
					...
					td = $j('<td class="fc-event-container"/>').append(seg.el);
...
DayGrid.mixin({
	...
	computeRowLevelLimit: function(row) {
		...
		function iterInnerHeights(i, childNode) {
			trHeight = Math.max(trHeight, $j(childNode).outerHeight());
		}
		...
		function emptyCellsUntil(endCol) { // goes from current `col` to `endCol`
			while (col < endCol) {
				...
				if (segsBelow.length) {
					...
					moreWrap = $j('<div/>').append(moreLink);
					td.append(moreWrap);
			...
			for (i = 0; i < levelSegs.length; i++) {
				...
				if (totalSegsBelow) { // do we need to replace this segment with one or many "more" links?
					...
					for (j = 0; j < colSegsBelow.length; j++) {
						moreTd = $j('<td class="fc-more-cell"/>').attr('rowspan', rowspan);
					...
					td.addClass('fc-limited').after($j(segMoreNodes)); // hide original <td> and inject replacements
					limitedNodes.push(td[0]);
				}
			}

			emptyCellsUntil(this.colCnt); // finish off the level
			rowStruct.moreEls = $j(moreNodes); // for easy undoing later
			rowStruct.limitedEls = $j(limitedNodes); // for easy undoing later
		}
	},
	...
	resliceDaySegs: function(segs, dayDate) {
		...
		var events = $j.map(segs, function(seg) {
			return seg.event;
		});
...
var TimeGrid = Grid.extend({
	...
	renderHelper: function(event, sourceSeg) {
		...
		this.helperEl = $j('<div class="fc-helper-skeleton"/>')
			.append(tableEl)
				.appendTo(this.el);
	},
	...
	renderFill: function(type, segs, className) {
		...
		if (segs.length) {
			...
			for (col = 0; col < segCols.length; col++) {
				colSegs = segCols[col];
				tdEl = $j('<td/>').appendTo(trEl);

				if (colSegs.length) {
					containerEl = $j('<div class="fc-' + className + '-container"/>').appendTo(tdEl);
...
TimeGrid.mixin({
	...
	renderFgSegs: function(segs) {
		...
		this.el.append(
			this.eventSkeletonEl = $j('<div class="fc-content-skeleton"/>')
	...
	renderSegTable: function(segs) {
		...
		for (col = 0; col < segCols.length; col++) { // iterate each column grouping
			...
			containerEl = $j('<div class="fc-event-container"/>');
			...
			trEl.append($j('<td/>').append(containerEl));
		}
...
var View = fc.View = Class.extend({
	...
	reportExternalDrop: function(meta, dropLocation, el, ev, ui) {
		...
		if (eventProps) {
			eventInput = $j.extend({}, eventProps, dropLocation);
	...
	documentMousedown: function(ev) {
		...
		if (this.isSelected && this.opt('unselectAuto') && isPrimaryMouseButton(ev)) {
			...
			if (!ignore || !$j(ev.target).closest(ignore).length) {
				this.unselect(ev);
			}
		}
	},
	...
	initHiddenDays: function() {
		...
		for (i = 0; i < 7; i++) {
			if (
				!(isHiddenDayHash[i] = $j.inArray(i, hiddenDays) !== -1)
			) {
				dayCnt++;
			}
		}
...
var Calendar = fc.Calendar = fc.CalendarBase = Class.extend({
	...
	getUnitViewSpec: function(unit) {
		...
		if ($j.inArray(unit, intervalUnits) != -1) {

			// put views that have buttons first. there will be duplicates, but oh well
			viewTypes = this.header.getViewsWithButtons();
			$j.each(fc.views, function(viewType) { // all views
...
function Calendar_constructor(element, overrides) {
	...
	function initialRender() {
		...
		content = $j("<div class='fc-view-container'/>").prependTo(element);
		...
		if (options.handleWindowResize) {
			...
			$j(window).resize(windowResizeProxy);
		}
	}
	...
	function destroy() {
		...
		if (windowResizeProxy) {
			$j(window).unbind('resize', windowResizeProxy);
		}
	}
	...
	function renderView(viewType) {
		...
		if (!currentView && viewType) {
			...
			currentView.setElement(
				$j("<div class='fc-view fc-" + viewType + "-view' />").appendTo(content)
			);
...
fc.datepickerLang = function(langCode, dpLangCode, dpOptions) {
	...
	$j.each(dpComputableOptions, function(name, func) {
	...
	if ($j.datepicker) {
		...
		$j.datepicker.regional[dpLangCode] =
			$j.datepicker.regional[langCode] = // alias
				dpOptions;

		// Alias 'en' to the default language data. Do this every time.
		$j.datepicker.regional.en = $j.datepicker.regional[''];

		// Set as Datepicker's global defaults.
		$j.datepicker.setDefaults(dpOptions);
	}
};
...
fc.lang = function(langCode, newFcOptions) {
	...
	$j.each(momComputableOptions, function(name, func) {
...
var momComputableOptions = {
	...
	dayOfMonthFormat: function(momOptions, fcOptions) {
		...
		format = format.replace(/^Y+[^\w\s]*|[^\w\s]*Y+$j/g, '');
	...
	mediumTimeFormat: function(momOptions) { // can't be called `timeFormat` because collides with option
		return momOptions.longDateFormat('LT')
			.replace(/\s*a$j/i, 'a'); // convert AM/PM/am/pm to lowercase. remove any spaces beforehand
	},
	...
	smallTimeFormat: function(momOptions) {
		return momOptions.longDateFormat('LT')
			.replace(':mm', '(:mm)')
			.replace(/(\Wmm)$j/, '($j1)') // like above, but for foreign langs
			.replace(/\s*a$j/i, 'a'); // convert AM/PM/am/pm to lowercase. remove any spaces beforehand
	},
	...
	extraSmallTimeFormat: function(momOptions) {
		return momOptions.longDateFormat('LT')
			.replace(':mm', '(:mm)')
			.replace(/(\Wmm)$j/, '($1)') // like above, but for foreign langs
			.replace(/\s*a$j/i, 't'); // convert to AM/PM/am/pm to lowercase one-letter. remove any spaces beforehand
	},
	...
	hourFormat: function(momOptions) {
		return momOptions.longDateFormat('LT')
			.replace(':mm', '')
			.replace(/(\Wmm)$j/, '') // like above, but for foreign langs
			.replace(/\s*a$j/i, 'a'); // convert AM/PM/am/pm to lowercase. remove any spaces beforehand
	},
	...
	noMeridiemTimeFormat: function(momOptions) {
		return momOptions.longDateFormat('LT')
			.replace(/\s*a$j/i, ''); // remove trailing AM/PM
	}
...
function populateInstanceComputableOptions(options) {
	$j.each(instanceComputableOptions, function(name, func) {
...
function Header(calendar, options) {
	...
	function renderSection(position) {
		var sectionEl = $j('<div class="fc-' + position + '"/>');
		var buttonStr = options.header[position];

		if (buttonStr) {
			$j.each(buttonStr.split(' '), function(i) {
				var groupChildren = $j();
				var isOnlyButtons = true;
				var groupEl;

				$j.each(this.split(','), function(j, buttonName) {
					var viewSpec;
					var buttonClick;
					...
					if (buttonName == 'title') {
						groupChildren = groupChildren.add($j('<h2>&nbsp;</h2>')); // we always want it to take up height
						isOnlyButtons = false;
					}
					else {
						...
						if (buttonClick) {
							...
							button = $j( // type="button" so that it doesn't submit a form
...
function EventManager(options) { // assumed to be a calendar
	...
	function fetchEventSource(source, fetchID) {
		_fetchEventSource(source, function(eventInputs) {
			var isArraySource = $j.isArray(source.events);
			var i, eventInput;
			var abstractEvent;
	...
	function _fetchEventSource(source, callback) {
		...
		if (events) {
			...
			if (url) {
				...
				pushLoading();
				$j.ajax($j.extend({}, ajaxDefaults, source, {
					data: data,
	...
	function buildEventSource(sourceInput) { // will return undefined if invalid source
		...
		if ($j.isFunction(sourceInput) || $j.isArray(sourceInput)) {
			...
			source = $j.extend({}, sourceInput); // shallow copy
		}
		...
		if (source) {
			...
			if ($j.isArray(source.events)) {
				source.origArray = source.events; // for removeEventSource
				source.events = $j.map(source.events, function(eventInput) {
	...
	function removeEventSource(source) {
		sources = $j.grep(sources, function(src) {
			return !isSourcesEqual(src, source);
		});
		// remove all client events from that source
		cache = $j.grep(cache, function(e) {
	...
	function getMiscEventProps(event) {
		...
		$j.each(event, function(name, val) {
	...
	function isMiscEventPropName(name) {
		return !/^_|^(id|allDay|start|end)$j/.test(name);
	}
	...
	function removeEvents(filter) {
		...
		cache = $j.grep(cache, filter, true); // inverse=true

		// Remove events from array sources.
		// This works because they have been converted to official Event Objects up front.
		// (and as a result, event._id has been calculated).
		for (i=0; i<sources.length; i++) {
			if ($j.isArray(sources[i].events)) {
				sources[i].events = $j.grep(sources[i].events, filter, true);
			}
		}
	...
	function clientEvents(filter) {
		if ($j.isFunction(filter)) {
			return $j.grep(cache, filter);
		}
		else if (filter != null) { // not null, not undefined. an event ID
			filter += '';
			return $j.grep(cache, function(e) {
				return e._id == filter;
			});
		}
	...
	function ensureVisibleEventRange(range) {
		...
		if (!range.end) {
			...
			range = $j.extend({}, range); // make a copy, copying over other misc properties
			...
			if (abstractEvent._recurring) {
				...
				while (date.isBefore(_rangeEnd)) {
					...
					if (!dowHash || dowHash[date.day()]) { // if everyday, or this particular day-of-week
						...
						event = $j.extend({}, abstractEvent); // make a copy of the original
						assignDatesToEvent(
							start, end,
	...
	function mutateEvent(event, newProps, largeUnit) {
		...
		$j.each(newProps, function(name, val) {
			if (isMiscEventPropName(name)) {
				if (val !== undefined) {
					miscProps[name] = val;
				}
	...
	function mutateEvents(events, clearEnd, allDay, dateDelta, durationDelta, miscProps) {
		...
		$j.each(events, function(i, event) {
			...
			$j.each(miscProps, function(name) {
			...
			$j.extend(event, miscProps, newProps); // copy over misc props, then date-related props
			backupEventDates(event); // regenerate internal _start/_end/_allDay

			undoFunctions.push(function() {
				$j.extend(event, oldProps);
				backupEventDates(event); // regenerate internal _start/_end/_allDay
			});
		});
	...
	function isExternalDropRangeAllowed(range, eventProps) {
		...
		if (eventProps) {
			eventInput = $j.extend({}, eventProps, range);
	...
	function isRangeAllowed(range, constraint, overlap, event) {
		...
		range = $j.extend({}, range); // copy all properties in case there are misc non-date properties
...
var AgendaView = fcViews.agenda = View.extend({
	...
	render: function() {
		...
		this.bottomRuleEl = $j('<hr class="fc-divider ' + this.widgetHeaderClass + '"/>')
